...
#include <fintrf.h>
    MODULE HSL_MATLAB
      ...
      integer, parameter, public :: mwl_ = C_BOOL ! based on matrix.h typedef
...
!---------------------------------
      ...
      INTERFACE matlab_to_fortran
         ...
         MODULE PROCEDURE matlab_to_fortran_scalar_logical
         ...
         MODULE PROCEDURE matlab_to_fortran_array1d_logical
         ...
         MODULE PROCEDURE matlab_to_fortran_array2d_logical
      ...
      INTERFACE fortran_to_matlab
         ...
         MODULE PROCEDURE fortran_to_matlab_scalar_logical
         MODULE PROCEDURE fortran_to_matlab_array1d_logical
         MODULE PROCEDURE fortran_to_matlab_array2d_logical
...
CONTAINS
   ...
   ! Following function is used to provide a workaround for g95 optimization bug
   ! By splitting it into a seperate function tree optimization doesn't crash.
   ! Function typecasts a mwPointer to a iso_c_binding C pointer
   type(C_PTR) function mp2c(mp)
      integer(mwp_), intent(in) :: mp

      mp2c = transfer(mp, mp2c) ! Typecast to C_PTR
   end function
...
!  nfields - number of components of the substructure
!  fieldnames - names of components of the substructure
!  ------------------------------------------------------
      SUBROUTINE MATLAB_create_substructure(struct, name, pr, nfields, &
            fieldnames)
         mwPointer ::struct, pr
         CHARACTER ( len = * ) :: name
         ...
         pr = mxCreateStructMatrix( 1_mws_, 1_mws_, nfields, fieldnames )
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      subroutine matlab_to_fortran_scalar_logical(mp, scalar, argname)
         mwPointer :: mp
         ...
         integer(int4_) :: tempint
         logical(mwl_), pointer :: boolptr
         ...
         dataptr = MATLAB_get_ptr(mp)
         select case(mxGetClassID(mp))
         case(mx_class_logical)
            ! Highly dubious, but /should/ work
            cdataptr = mp2c(dataptr)
            call C_F_POINTER(cdataptr, boolptr)
            scalar = boolptr
         case default
            call matlab_to_fortran_scalar_int4(mp, tempint, argname)
            scalar = (tempint.ne.0)
         end select
      end subroutine matlab_to_fortran_scalar_logical
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      subroutine matlab_to_fortran_array1d_logical(mp, array1d, n, argname)
         mwPointer :: mp
         logical, dimension(:), allocatable, intent(out) :: array1d
         mwSize :: n
         character(len=*), intent(in) :: argname
         ...
         mwPointer :: dataptr
         type(C_PTR) :: cdataptr
         integer(int4_), dimension(:), allocatable :: tempint
         logical(mwl_), dimension(:), pointer :: boolptr
         integer :: st
         ...
         case(mx_class_logical)
            ! Highly dubious, but /should/ work
            cdataptr = mp2c(dataptr)
            call C_F_POINTER(cdataptr, boolptr, shape = (/ n /))
            array1d(:) = boolptr(:)
         case default
            call matlab_to_fortran_array1d_int4(mp, tempint, n, argname)
            array1d(:) = (tempint(:).ne.0)
         end select
         ...
         write(errmsg, "(3a)") &
            "Error processing argument ", argname, ". Allocation failed."
         call MATLAB_context_error("hsl_matlab:AllocFail", errmsg)
      end subroutine matlab_to_fortran_array1d_logical
      ...
      subroutine matlab_to_fortran_array2d_logical(mp, array2d, m, n, &
            argname)
         mwPointer :: mp
         logical, dimension(:,:), allocatable, intent(out) :: array2d
         mwSize, intent(out) :: m, n
         character(len=*), intent(in) :: argname
         ...
         case(mx_class_logical)
            ! Highly dubious, but /should/ work
            cdataptr = mp2c(dataptr)
            call C_F_POINTER(cdataptr, boolptr, shape = (/ m, n /))
            array2d(:,:) = boolptr(:,:)
         case default
            call matlab_to_fortran_array2d_int4(mp, tempint, m, n, argname)
            array2d(:,:) = (tempint(:,:).ne.0)
         end select
         ...
         write(errmsg, "(3a)") &
            "Error processing argument ", argname, ". Allocation failed."
         call MATLAB_context_error("hsl_matlab:AllocFail", errmsg)
      end subroutine matlab_to_fortran_array2d_logical
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      mwPointer function fortran_to_matlab_scalar_logical(scalar)
         logical, intent(in) :: scalar

         fortran_to_matlab_scalar_logical = &
            fortran_to_matlab_array1d_logical( (/scalar/) )
      end function fortran_to_matlab_scalar_logical
      mwPointer function fortran_to_matlab_array1d_logical(array1d, n)
         logical, dimension(:), intent(in) :: array1d
         mwSize, optional :: n

         integer(int4_), dimension(:), allocatable :: int_array
         mwSize :: len

         len = size(array1d)
         if(present(n)) len = n

         allocate(int_array(len))
         where(array1d(:))
            int_array(:) = 1
         elsewhere
            int_array(:) = 0
         ...
         fortran_to_matlab_array1d_logical = fortran_to_matlab(int_array, n=n)
      end function fortran_to_matlab_array1d_logical
      mwPointer function fortran_to_matlab_array2d_logical(array2d, m, n)
         logical, dimension(:,:), intent(in) :: array2d
         mwSize, optional :: m
         mwSize, optional :: n
         ...
         fortran_to_matlab_array2d_logical = fortran_to_matlab(int_array, m=m, &
            n=n)
      end function fortran_to_matlab_array2d_logical
